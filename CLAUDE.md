...
> ⚠️ **启动只有两条入口**：headless = 终端 `npm start`；macOS 还可走 `desktop/` 的 CCM.app。桌面端占着 3000 时**勿再手动 `npm start`**。改配置/代码后，桌面端菜单里 server 一行点「重启」，headless 重启那个进程。**例外**：工作区列表支持热加载，改完即生效、免重启（`ccm.config.json` 的 `WORKDIRS` 或旧版 `workdirs.json`，server 监听文件变化，被移除目录上的已开会话继续运行、仅拒新开；手机上「添加文件夹」写的也是这份内联 `WORKDIRS`）。哪些项热加载由 schema 的 `reload` 标记决定，当前只有 `WORKDIRS`。
...
npm run test:e2e   # Playwright 移动端 UI 回归（零外部依赖 mock server）；test:visual 是兼容别名
                   ...
                   # 要全量走下面的 test:e2e:parallel（10 核缺省 8 片 ~105s）；开发循环则只跑相关的那几条：
...
npm run test:e2e:parallel  # 同一批用例分片并行（分片数按核数自适应，CCM_E2E_SHARDS=N 可覆盖）。
                   ...
                   # 2026-09-23 实测（10 核 · 52 个 spec · 345 条 · 串行总和 851s）：4 片 207s ·
                   # 8 片 105–116s（三轮 1035 条零失败）；9/18 另测过 5 片 ~180s、6 片 ~150s。
                   # **缺省 = min(8, 核数-2)**，10 核即 8 片。4 片时只用了 2.1 个核：瓶颈是分片数，
                   # 不是 CPU。地板≈最大那个 spec 文件自己的耗时——同一文件不跨分片，
                   # workspace-sessions-sidebar 单跑实测 108s（9/18），所以 8 片已经触底、再加没有收益。
                   # （别拿 .e2e-durations.json 里的值当地板：那是【上一轮】的观测，含当轮负载，
                   # 实测偏大——9/18 那次缓存记 117.9s 而单跑只要 108s，照抄会算出「8 片比地板还快」。）
                   # 缺省曾停在 4，是 flaky 选的：并行度越高，「等固定时间窗」的用例越容易超时。
                   # 当时点名的 task-progress P0-17k/17i 都已改成不赌窗口；P0-SYNC-ACK-TIMEOUT 记过
                   # 「4 片下偶发红」，根因仍未定位。遇到疑似时序假红，先用 CCM_E2E_SHARDS=4 重跑对照。
