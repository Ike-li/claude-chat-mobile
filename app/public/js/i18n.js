...
export const EN_DICT = Object.freeze({
  ...
  '设置与状态': 'Settings & status',
  '打开设置与状态': 'Open settings & status',
  '通知 · 设备信任 · 语言 · 诊断': 'Notifications · device trust · language · diagnostics',
  '下拉或点外侧关闭 · 不含本会话的模型与权限': "Swipe down or tap outside · this session's model & permissions live elsewhere",
  // L2 页内的组标题与作用域 chip
  '现在': 'Now',
  '怎么提醒我': 'How to alert me',
  '📱 仅此设备': '📱 This device only',
  '🖥 整机': '🖥 Whole machine',
  '锁屏带内容预览': 'Content preview on lock screen',
  '运行状态': 'Runtime',
  // statusline 桥（与 hooks 桥并列的第二个 CLI 桥）
  '终端状态栏': 'Terminal status line',
  '已停用（CLI_STATUSLINE_BRIDGE=off）': 'Disabled (CLI_STATUSLINE_BRIDGE=off)',
  '读不出 ~/.claude/settings.json 的安装记录；在电脑上跑 npm run statusline:status 查看': 'Cannot read the install record in ~/.claude/settings.json; run npm run statusline:status on the computer',
  '开启后，你在电脑终端里跑的会话，模型 / 额度 / 上下文用量也会同步到手机': 'Once on, sessions you run in the terminal also sync model / quota / context usage to your phone',
  '终端会话的模型 / 额度 / 上下文用量同步到手机': 'Syncs terminal sessions\' model / quota / context usage to your phone',
  '开启终端状态栏同步？': 'Turn on terminal status line sync?',
  '关闭终端状态栏同步？': 'Turn off terminal status line sync?',
  '会接管 ~/.claude/settings.json 里的 statusLine 命令，并原样透传你原来的输出（原命令会先备份）。已在跑的终端会话需重开才生效。': 'Takes over the statusLine command in ~/.claude/settings.json and passes your original output through unchanged (the original command is backed up first). Running terminal sessions need a restart.',
  '会把 statusLine 命令恢复成你原来的那条，备份记录随之删除。': 'Restores your original statusLine command and removes the backup record.',
  '开启后，你在电脑终端里跑的会话，模型 / 额度 / 上下文用量也会同步到手机。它接管 ~/.claude/settings.json 的 statusLine 命令并原样透传你原来的输出。': 'Once on, sessions you run in the terminal also sync model / quota / context usage to your phone. It takes over the statusLine command in ~/.claude/settings.json and passes your original output through unchanged.',
  '宿主机上的 claude': 'claude on the host',
  'MCP 服务器': 'MCP servers',
  '个': '',
  '个可用': 'available',
  '当前工作区加载的技能': 'Skills loaded for this workspace',
  '状态未知': 'status unknown',
  '先跑这个': 'Start here',
  '⚙ 全部配置 · 端口 / 工作区 / 推送 →': '⚙ All settings · port / workspaces / push →',
  '在下方「已受信任的设备」里核对时，认准这一串就是你手上这台。': 'When checking the trusted device list below, this string identifies the device in your hand.',
  // L1 目录六行（logic/general-nav.js 算出，非静态文案）
  '通知': 'Notifications',
  '接入与设备': 'Access & devices',
  '行为与开关': 'Behavior & toggles',
  '排查': 'Troubleshoot',
  '帮助': 'Help',
  '推送未开启': 'Push off',
  '推送已开': 'Push on',
  '台已信任': 'trusted',
  '台待批': 'pending',
  '有设备在等你批': 'A device is waiting for approval',
  '台设备在等你批': 'devices waiting for approval',
  '收起这张面板，在顶部卡片上「准入 / 拒绝」': 'Closes this panel so you can Approve / Deny on the card at the top',
  '锁屏预览': 'Lock-screen preview',
  '扫码接新设备': 'Scan to add a device',
  '本机指纹': 'Device fingerprint',
  '终端推送': 'Terminal push',
  // 桥处于 drifted / unknown 时的第三档：既不是开也不是关（详见 logic/general-nav.js 的 bridgeItem）
  '待查': 'unknown',
  '已运行': 'Up',
  ...
  '码里含访问令牌，扫到即可进入。展开前请确认周围没有别人能看到屏幕。': 'The code contains your access token — anyone who scans it gets in. Make sure nobody else can see your screen.',
  '⚠️ 二维码等同于一把钥匙：任何人扫到都能以你的身份进来。确定现在显示吗？': '⚠️ This QR code is a key: anyone who scans it enters as you. Show it now?',
  '显示': 'Show',
  '立即隐藏': 'Hide now',
  '码里含访问令牌，扫一下即可接入': 'Contains your access token — scan to connect',
  '该域名受 Cloudflare Access 保护，码里不含令牌': 'This hostname is protected by Cloudflare Access, so the code carries no token',
  '秒后自动隐藏': 'seconds until it hides',
  '二维码生成失败': 'Could not generate the QR code',
  '直接放行 · 不弹审批': 'Allowed · no prompt',
  '直接拒绝': 'Denied',
  '每次都问': 'Always ask',
  '来自 ~/.claude/settings.json 与本工作区的 .claude/settings.local.json，与你在电脑终端里用的是同一份。这里只读——要改请在电脑上编辑那两个文件。': 'From ~/.claude/settings.json and this workspace\'s .claude/settings.local.json — the same rules your terminal uses. Read-only here; edit those files on the computer to change them.',
  '端口与工作区': 'Ports & workspaces',
  '安全体检': 'Security check',
  '服务日志': 'Service log',
  'server 进程自己的 stdout / stderr —— 「为什么起不来」的答案在这里': "The server process's own stdout / stderr — where 'why won't it start' is answered",
  '会话日志 →': 'Session log →',
  '本会话的交互与状态机时间线 —— 「消息到底发出去没有」。前端日志也在这张抽屉的「诊断」过滤里': "This session's interaction and state-machine timeline — 'did the message actually go out'. Frontend logs are in the same drawer, under the Diagnostics filter",
  '安全日志 →': 'Security log →',
  '谁在什么时候被限速锁定过、推送投递失败过 —— 在「服务状态」面板里': 'Who got rate-limit locked out and when, and which pushes failed to deliver — lives in the Service status panel',
  '后两条会切去另一张面板，从那里返回仍回到这一页。': 'The last two switch to another panel; going back from there returns you here.',
  '日志': 'Logs',
  '未知路径': 'Unknown path',
  '文件是空的（服务刚起来，或日志被轮转过）': 'The file is empty (the service just started, or the log was rotated)',
  '只显示文件末尾部分': 'Showing the tail of the file only',
  '含真实路径与错误原文（未脱敏）——投屏时注意': 'Contains real paths and raw error text (not redacted) — mind screen sharing',
  '会话日志': 'Session log',
  '新设备怎么批': 'Approving a new device',
  '令牌在哪': 'Where is the token',
  '连不上': "Can't connect",
  '推送未接通': 'Push offline',
  '去开启': 'Set up',
  ...
  '已经有一台手机能用了？在那台上打开设置 →「接入与设备」→「📷 显示接入二维码」，新手机扫一下就进来了，不用手输令牌。否则打开': 'Already have one phone working? On that phone open Settings → "Access & devices" → "📷 Show access QR code" and scan it with the new phone — no typing the token. Otherwise open',
  ...
  '在电脑上的': 'Look in',
  '里找': 'on your computer for',
  '这一项（旧版装的是': '(older installs use',
  '），或者跑': '), or run',
  '打一张二维码直接扫。启动日志只有第一次打印完整 URL，之后为防泄露只显示掩码，别照着它输。清除浏览器数据会一并清掉令牌，届时重输一次即可。': 'to print a QR code you can scan directly. The startup log prints the full URL only the first time; after that it shows a masked token — do not type that one in. Clearing browser data wipes the token too, so just enter it again when that happens.',
  '🆕 新设备怎么获批': '🆕 How a new device gets approved',
  '除了本机直连、以及已开 Cloudflare Access 的域名，其余路径（局域网 IP、Tailscale、没配 Access 的隧道域名）新设备都要批一次。你已登录的设备上会弹「新设备请求接入」，点「准入」即可——': 'Every path needs a one-time approval for a new device, except direct localhost access and a domain already protected by Cloudflare Access. That includes LAN IPs, Tailscale, and tunnel domains without Access. A device you are already signed in on shows "New device requesting access" — just tap "Approve", ',
  // 末尾空格有意为之：紧跟其后的是 <code>，中文冒号后不留空格、英文冒号后要留
  '。兜底命令：': '. Fallback command: ',
  '🔄 连不上怎么办': "🔄 If you can't connect",
  '多半是令牌被清——重输即可。若提示设备未获批，按上一条批一次。切后台/息屏断开会在回到前台时自动重连，无需手动刷新。': 'Most often the token was cleared — just enter it again. If it says the device is not approved, approve it once as described above. Disconnects from backgrounding or screen-off reconnect on their own when you come back; no manual refresh needed.',
  ...
  '本机设备指纹': 'This device\u2019s fingerprint',
  '完整 ID': 'Full ID',
  '已受信任的设备': 'Trusted devices',
  '吊销后该设备立刻失去访问权，正用设备令牌连着的那条连接会被断开。这不是拉黑，之后仍可重新申请。':
  ...
  '已连接的文件夹': 'Connected folders',
  '一次性目录，不属于任何项目': 'A throwaway folder that isn\'t part of any project',
  '添加文件夹': 'Add folder',
  '从家目录里选一个文件夹连接进来': 'Pick a folder in your home folder to connect',
  '查看子文件夹': 'Show subfolders',
  '这个文件夹不在家目录里，看不了它的子文件夹': 'This folder isn\'t inside your home folder, so its subfolders can\'t be browsed',
  '添加这个文件夹': 'Add this folder',
  '在这里开始会话': 'Start a session here',
  '新建文件夹': 'New folder',
  ...
  '这是 Claude 或本应用自己的目录，不能添加': 'This is Claude\'s or this app\'s own folder and can\'t be added',
  '这是 git worktree，跟随所属仓库；请添加仓库本身': 'This is a git worktree; it follows its repository — add the repository instead',
  '已经连接': 'Already connected',
  '文件夹不存在': 'Folder not found',
  '不是文件夹': 'Not a folder',
  '没有 ccm.config.json，手机上添加不了；请在电脑上运行 npm run setup': 'No ccm.config.json, so folders can\'t be added from the phone — run npm run setup on your computer',
  '工作区列表来自环境变量 WORK_DIRS，手机上改不了': 'The folder list comes from the WORK_DIRS environment variable and can\'t be changed from the phone',
  '配置文件读不出来，已拒绝改写': 'The config file couldn\'t be read, so it was left untouched',
  '写入配置文件失败': 'Couldn\'t write the config file',
  '已写入配置，但没有生效': 'Saved to the config, but it didn\'t take effect',
  '配置校验没通过': 'The config didn\'t pass validation',
  '同名文件夹已存在': 'A folder with that name already exists',
  '名字不能以 . 开头': 'Names can\'t start with .',
  '名字里不能有斜杠': 'Names can\'t contain slashes',
  '名字里有不可见字符': 'The name contains invisible characters',
  '名字太长': 'Name too long',
  '请输入名字': 'Enter a name',
  '这里不能浏览或新建文件夹': 'You can\'t browse or create folders here',
  '新建文件夹失败': 'Couldn\'t create the folder',
  '读不出这个文件夹': 'Couldn\'t read this folder',
  ...
  '分叉失败': 'Fork failed',
  // —— /rewind 面板（app/rewind-command.js）与回退结果 ——
  // 三个模式名与终端 /rewind 第二步逐字一致；其余沿用上面「回退 = revert」的叫法。
  '把代码和/或对话恢复到某条消息发出之前……': 'Restore the code and/or conversation to the point before one of your messages…',
  '正在读取会话…': 'Reading the session…',
  '这个会话还没有可回退的轮次': 'This session has no turns to revert yet',
  '代码改动待确认': 'Code changes not known yet',
  '个文件改动': 'file(s) changed',
  '无代码改动': 'No code changes',
  '会话首轮，只能恢复代码': 'First turn of the session — code only',
  '（空消息）': '(empty message)',
  '确认要恢复到这条消息发出之前：': 'Confirm you want to restore to the point before this message:',
  '正在计算影响面…': 'Working out what will change…',
  '回退不影响手动或通过 bash 改过的文件。': 'Reverting doesn\'t affect files you edited by hand or through bash.',
  '无法读取回退预览': 'Couldn\'t load the revert preview',
  '对话将分叉出新会话，原会话完整保留。': 'The conversation will be forked into a new session; the original stays intact.',
  '这是会话首轮，无法分叉对话，只能恢复代码。': 'This is the session\'s first turn: the conversation can\'t be forked, only the code can be restored.',
  '这一轮既没有可恢复的文件，也无法分叉对话。': 'This turn has no files to restore, and the conversation can\'t be forked.',
  '找不到这一轮的文件快照，只能恢复对话。': 'No file snapshot was found for this turn, so only the conversation can be restored.',
  '这一轮没有代码改动，只能恢复对话。': 'This turn made no code changes, so only the conversation can be restored.',
  '这一轮没有代码改动。': 'This turn made no code changes.',
  // 「将恢复 N 个文件：a、b」「其中 N 个文件有未提交的改动…：a、b」由片段与数字拼成，译文按英文语序分给各片段
  '将恢复': 'Will restore',
  '个文件：': 'file(s): ',
  '其中': 'Of these,',
  '个文件有未提交的改动，回退会覆盖它们：': 'file(s) have uncommitted changes the revert will overwrite: ',
  '恢复代码和对话': 'Restore code and conversation',
  '只恢复对话': 'Restore conversation',
  '只恢复代码': 'Restore code',
  '请求超时，请重试': 'Request timed out — try again',
  '还没有会话可回退': 'No session to revert yet',
  '已回退 {n} 个文件（对话未改动）': 'Reverted {n} file(s) (the conversation is unchanged)',
  '已分叉出回到那一刻的新会话（原会话保留，文件未改动）': 'Forked a new session from that point (the original is kept, files unchanged)',
  ...
  '关闭推送': 'Turn off',
  '已关闭': 'Turned off',
  '这台设备不会再收到推送。随时可以重新开启。': 'This device will no longer receive push notifications. You can turn them back on anytime.',
  ...
  '🔕 已关闭推送通知': '🔕 Push notifications turned off',
  '⚠️ 关闭推送失败，请重试': '⚠️ Could not turn off push notifications — please try again',
