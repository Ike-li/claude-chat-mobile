...
export const EN_DICT = Object.freeze({
  ...
  '找不到这一轮的文件快照': 'No file snapshot found for this turn',
  '这一轮没有经 Claude 编辑过的文件——Bash 命令改动的文件不在快照范围内，所以没有可恢复的内容。': 'Claude didn\'t edit any files in this turn — files changed by Bash commands aren\'t snapshotted, so there\'s nothing to restore.',
  '找不到这条消息对应的文件快照，没有可恢复的文件。': 'No file snapshot exists for this message, so no files can be restored.',
  '如果你要的是清掉这条消息之后的对话，可以改用「分叉」：复制一个到此为止的新会话，不动任何文件。': ' If what you want is to drop the conversation after this message, use Fork instead: it copies a new session up to this point and touches no files.',
  '改用分叉': 'Fork instead',
