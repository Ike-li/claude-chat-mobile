// metrics.js —— 运行时指标收集器 + 状态探针（MetricsCollector/StateProbe）
//
// 纯内存、重启清零（运行时易失态，与 RingBuffer/RateLimitState 同层）——指标是当前进程的运行统计，
// 跨重启无意义，不落盘、无 CCM_ 隔离口。快照只经**鉴权保护**的端点暴露（src/server/app.js 的 /metrics，守
// "不开无鉴权数据端点"），不推 Prometheus 文本、不主动遥测。
//
// 边界（OQ-09，见 docs/hard-rules.md，别混淆）：本模块只收"系统管道健康"指标（会话数/事件速率/补齐率/限速数/推送成功率）——
// OQ-09 拒绝的是"授权闭环时延"这类**人机/价值指标**的埋点遥测，不在此列
// 本模块不采集任何审批时延/闭环时长。
...
// 瞬时值设置。用于需要"事件驱动更新、消费者随时读最新"的瞬时指标。
// 注：能在 /metrics 拉取时实时算准的当前值（如活跃会话数=agents.size）直接实时算、不走 gauge 中转更准；
// gauge 留给未来那类"变化点分散、拉取时算不出全貌"的瞬时指标。API 完整性保留 + 单测覆盖。
