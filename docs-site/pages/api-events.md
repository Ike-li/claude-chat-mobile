...
| 人机交互与审批 | permission_request 、 question 、 request_resolved 、 prompt_suggestion | 工具审批卡片、人机多选提问、输入建议与决议完成 |
| 核心状态同步 | init 、 instances 、 mirror_state 、 status_line 、 rewind_applied | 会话初始化、活跃实例全景、只读镜像锁、动态状态行与回退生效通知 |
...
| 元数据与审计 | session_log 、 diag_log 、 history_append 、 user_message 、 session_recap | 会话日志追加、诊断日志、镜像增量消息、本地回显与会话摘要 |
...
| 代码与 Git 集成 | git:status git:diff git:branches hooks:setup |
| 后台任务管理 | task:stop task:output tool:preview tool:full subagent:flow |
| 系统运维与诊断 | service:status doctor:run dev:restart logs:get logs:server logs:clientError audit:get push:test config:refresh env:get env:set statusline:setup |
