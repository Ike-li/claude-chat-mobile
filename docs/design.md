...
# 终端 2：
node scripts/smoke.js              # M1 行走骨架（A1/A2/A4/A6 + 会话切换）
...
```bash
npm run check              # JS 语法 + 文档一致性 + visual mock registry guard（零 token、最快）
...
| ~~CI~~（已破例，2026-06-28） | 仓库 public 后加了 GitHub Actions（`.github/workflows/test.yml`）：`unit-test` 跑 `npm test`，`visual-e2e` 跑 `npm run test:visual`。这是唯一例外：不引入 lint / 构建 / 真实 Claude smoke；需本机 Claude 的集成路径在 CI 中跳过或显式 opt-in |

**最终成功度量**：连续一周，晚上想让 claude 干活时，不再需要走到电脑前。

## 7. 分支与发布模型

> 运行时是 n=1（§1），但源码是公开开源产品——别人会 clone/fork/贡献。分支模型服务后者：把"稳定/已发布"和"开发中"分开。
...
- **CI**（`.github/workflows/test.yml`）— `push` 到 `master` / `dev` 与所有 PR 都跑 `npm test` 和 mock-only `npm run test:visual`。
- 自己的常驻服务（见 `deployment.md`）想尝鲜 pull `dev`、想稳 pull `master`，不强制。

## 8. Workflow / 子 Agent 的 Web 端行为

> Claude 的 workflow（工作流）会 spawn 子 agent，产生多路并发的消息流。本节描述它们在 web 端的显示行为，以及 web 端续接一个 CLI 正在跑的 workflow 会话时能看到什么、不能看到什么。
>
> 📎 **接口参考**：前后端完整接口（HTTP 端点、Socket.IO 双向事件、内部模块 API）见 [interfaces.md](interfaces.md)；出向 `agent:event` 24 类的可执行契约见 [event-contract.md](event-contract.md)。

### 运行中（web 端发起的 workflow）

- **子 agent 内部流不外露**：带有 `parent_tool_use_id` 的消息（subagent 的流式输出、中间 assistant/user 记录）在后端统一过滤，不进入主会话消息流——与终端行为等价：终端也不把 workflow 子进程的逐字输出合并到主 tty 上。
...
- **历史回显过滤**：`history.js` 读取 CLI JSONL 时跳过 `type:mode`（web 自身 resume 写入的）、`<task-notification>` 系统注入、以及 content 全为 tool_use/tool_result block 的纯工具条目——历史里只剩主线文字气泡和工具调用卡片。
- **subagent 记录基本不回显**：运行期 `agent.js` 按 `parent_tool_use_id` 跳过子 agent 消息；历史侧 `history.js` 无此字段，靠只收主线 user/assistant 文字、滤掉纯 tool_use/tool_result 空正文条目近似达成（子 agent 多为工具活动、空正文被滤；带正文的子 agent 记录理论上可能漏回显）。
