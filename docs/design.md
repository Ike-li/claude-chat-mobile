...
| ~~CI~~（已破例，2026-06-28） | 仓库 public 后加了只跑 `npm test` 的 GitHub Actions（`.github/workflows/test.yml`）作为 master/dev push 与外部 PR 的 test 门禁——唯一例外，不引入 lint / 构建 / 真实 E2E。集成测试 `server.test.mjs` 在 CI 环境跳过（preflight 需本机 claude CLI） |
...
- **CI**（`.github/workflows/test.yml`）— `push` 到 `master` / `dev` 与所有 PR 都跑 `npm test`。
...
- **subagent 记录不回显**：CLI JSONL 里子 agent 的消息同样带 `parent_tool_use_id`，历史读取时被过滤，与运行期行为一致。
