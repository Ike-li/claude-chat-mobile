...
- **subagent 记录不回显**：运行期 `agent.js` 按 `parent_tool_use_id` 跳过子 agent 消息；历史侧 `history.js` 按磁盘 JSONL 的 `isSidechain` 标记过滤（`parent_tool_use_id` 是 SDK 运行时流字段、不落盘，故防御性一并挡），两侧等效——含带正文的子 agent 记录也不回显。
