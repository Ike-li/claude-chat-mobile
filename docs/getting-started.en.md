...
### Option A: source archive (you just want to run it)
...
The archive is generated by GitHub on the fly (`/archive/refs/heads/master.tar.gz`) and is about 1.5 MB. `master` only moves on release, so it is the latest release; to pin a version use `/archive/refs/tags/vX.Y.Z.tar.gz`, which unpacks into `claude-chat-mobile-X.Y.Z`. Trimming follows `export-ignore` in the repository's `.gitattributes`: the test tree, the Docker test environment, and the maintainer gates are left out, while the server, the setup wizard, the preflight checks, device approval, the desktop app, and both CLI bridges are all kept.

`package.json` is kept as-is: `npm run` still lists `test` / `check` / `lint` / `mutate` and friends, but the test tree and gates they refer to are not in the archive, so they cannot run; `devDependencies` is still there too, and `--omit=dev` skips it. Use option B when you need `npm test` / `npm run check` / `npm run lint`.
...
Both paths use `npm ci --omit=dev`: it reproduces the dependency tree from `package-lock.json` exactly without rewriting the lock, and installs only runtime dependencies, so no Playwright browsers are downloaded. To run the tests in the full repository, run `npm ci` once more without that flag.
...
- **Can't be connected**: your home folder itself and the disk root (the config file rejects them too). `~/.claude` and this app's own data directory can't be added from the phone, and connecting a parent (say `~/code`, with this app inside it) doesn't turn them into session directories — unless you list them in `WORKDIRS` explicitly in the config file. A linked worktree can't be added on its own; add the repository it belongs to.
- **Add folders from the phone**: tap the folder chip on the new-session page, or "＋ Add folder" at the top of the drawer — it browses folder names inside your home folder only, and greys out the ones that can't be added with the reason. Adding writes the folder back into `WORKDIRS` in the config file and takes effect immediately. Two cases can't be changed from the phone: the folder list comes from environment variables (the two legacy keys below), or there is no `ccm.config.json` (legacy `.env` only) — run `npm run setup` on the computer to create one.
