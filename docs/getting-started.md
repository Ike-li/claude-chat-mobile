...
- **不能连接的**：家目录本身、磁盘根（配置文件里也写不进去）。`~/.claude` 与本应用的数据目录手机上加不了；连了它们的上级目录（比如装着本应用的 `~/code`）时，它们也不会顺带变成会话目录——除非你在配置文件里把它们显式写进 `WORKDIRS`。linked worktree 不能单独添加，添加它所属的仓库即可。
- **手机上可以直接添加**：新会话页点文件夹胶囊，或抽屉顶部「＋ 添加文件夹」——只浏览家目录里的目录名，不能加的会置灰并写明原因。添加会写回配置文件里的 `WORKDIRS` 并立即生效。两种情况手机上改不了：工作区列表来自环境变量（下面那两个旧键）；没有 `ccm.config.json`（只用旧版 `.env`）——先在电脑上运行 `npm run setup` 生成。
