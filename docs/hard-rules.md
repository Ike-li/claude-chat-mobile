...
| 必须 gitignore | 与 `.env` 同等敏感且本仓 **public**；规则在 `.gitignore`（`ccm.config.json` / `ccm.config.*.json` / `ccm.config.json.*` 三条）。**由 `CONFIG-03` 钉住**（`tests/invariants/config-gitignored.test.mjs`，逐条跑 `git check-ignore` + 一条反向锚点）——2026-09-17 之前这里确实没有任何测试或门禁，删掉那三行不会让任何东西变红。gitleaks 钩子不是替代品：它扫的是「内容像不像凭据」，`CONFIG-03` 管的是「路径会不会被收进来」 |
