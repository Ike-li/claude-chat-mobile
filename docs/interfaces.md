...
| `service:status` | `{}` | `{ok, startedAt, versions:{server,cli,sdk}, deliveryFailure, rateLimitLockout, clientError, logging:{interactions,sdkDebug,stderr}, timestamp}`（服务状态面板，判定化：一次 ack 拼齐基础 + 带 24h 时效窗的告警判定（投递健康/登录限速锁定/前端错误，超窗自动退场为 null）+ 日志开关可见性；不带裸计数器——原始计数留 `GET /metrics` 巡检端点；面板开着时 5s 重拉） |
