...
`canUseTool(toolName, input, {signal})` 回调被 SDK 在执行高风险工具**前**调用。**"高风险"判定权归属已决(OQ-07,承接 HLD AD-7)**:何时触发本回调完全由上游 SDK 决定,本产品不自建风险分类体系、不提供自定义规则的配置项;`op` 携带的 `risk` 字段是展示性标签(供 §3.2.5 聚合展示分组,不驱动排序或拦截逻辑),不是产品自行计算的判定结果:
...
**参数(OQ-03 已决,采纳为可配置默认)**:threshold=8、baseBackoff=500ms、maxBackoff=30s、lockMs=15min、decayMs=15min(手滑容忍 + 暴破不经济 + 久未失败自动原谅)。告警的具体渠道/阈值不在本次决策范围,留实现阶段按可观测性需要定(§3.7 已有"计入审计+告警"的方向性要求,渠道选型不影响本节算法)。
...
- **推出节奏(OQ-04 已决)**:首次部署**默认静态令牌 + TOFU**,不强制先完成 WebAuthn 注册——呼应 FR-16/17"30 分钟部署"目标(不因强认证拦首次体验)与自托管产品惯例(不预设"部署即完美")。UI 在会话内**持续提示**"建议升级到设备绑定强认证"(非一次性弹窗、非强制截止时限),直到用户完成注册;静态令牌路径本身不因此降级为"临时/受限"——两者是**并列的持久可用路径**,仅安全强度不同(设备绑定 vs 共享口令),由操作者自行决定何时升级或是否长期停留静态令牌(如部署在纯 IP/自签环境、WebAuthn 结构性不可用时)。
...
- **payload 最小化(承接 HLD v2.9 AD-9 / PRD OQ-08)**:`NotifPayload = {kind:'approval'|'input'|'finished', cwdBase, deepLink, ts}`——**不含命令/参数/完整路径/消息正文**(`cwdBase` 仅目录尾段)。**按最弱通道设计**:标准 Web Push 有端到端加密(RFC 8291,push service 不见明文),FallbackChannel 第三方无此保证、按明文对待——故两通道共用同一份最小 payload,正文回 app 内经鉴权取;锁屏预览泄露面同由最小化缓解。**已决(OQ-08)**:`cwdBase` 默认显示,不提供隐藏配置项——本身仅目录尾段(非完整路径),泄露面已属最小化范畴,不为此再加一层配置复杂度(呼应瘦中转:不为边际暴露面加开关)。
