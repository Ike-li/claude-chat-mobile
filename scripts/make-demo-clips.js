// make-demo-clips.js —— 用视觉 mock UI 录制 6 段高分辨率手机演示片段（供外部视频工程剪辑用）。
// 零外部二进制依赖（除系统 ffmpeg）：Playwright Chromium(deviceScaleFactor:3 高分屏截帧序列) + ffmpeg 编码。
// 场景走 tests/e2e/mock/scenarios/demo.js 的 demo:* 中文场景（真实感中文 demo 文案，非 test:* 英文 mock）。
// 用法：node scripts/make-demo-clips.js   产物：CCM_CLIPS_OUT_DIR 下 6 个 <name>.mp4 + clips.json
...
const OUT_DIR = process.env.CCM_CLIPS_OUT_DIR || join(ROOT, 'tmp-clips');
...
async function main() {
  ...
  try {
    const page = await browser.newPage({
      ...
      isMobile: true,
      hasTouch: true,
      deviceScaleFactor: 3, // 375x812 -> 1125x2436 高分辨率（对齐旧素材 1124x2436）
    });
    ...
    const recordClip = async (name, action) => {
      ...
      // 1125(=375*3) 是奇数，H.264 要求宽高偶数；顺带缩到旧素材同尺寸 1124x2436，AR 常量不变。
      // -g/-keyint_min 强制每秒一个关键帧：静止画面居多的片段（如 approval 定格审批弹窗）
      // 若关键帧过稀疏，HyperFrames 渲染管线 seek 该片段会画面冻结/失败。
    ...
    // 5) tabs —— 多仓库并行：文字介绍后展开会话抽屉看两个新工作区（各自运行中/待审批状态图标）
    await reset();
