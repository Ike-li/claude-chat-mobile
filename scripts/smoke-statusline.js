...
//   node scripts/smoke-statusline.js --off    # e2e 禁用分支：server 以 WEB_STATUSLINE=off 跑着时执行
import { io } from 'socket.io-client';
import { dirname } from 'node:path';
import { fileURLToPath } from 'node:url';

const HERE = dirname(dirname(fileURLToPath(import.meta.url))); // 项目根（git 仓库）
const results = [];
...
if (process.argv.includes('--unit')) {
  const { buildWebStatusLine, gitStatus } = await import('../statusline.js');
  ...
  const git = await gitStatus(HERE);
