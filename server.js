...
function setMirror(readonly, sessionId, force = false, stale = false) {
  if (!force && readonly === mirrorReadonly && stale === mirrorStale) return; // 仅变化时广播；force=视图切换入场时强制发权威态（消除切换空窗）
  mirrorReadonly = readonly; mirrorStale = stale;
  io.to('approved').emit('agent:event', { // SEC-01：仅广播给已批准设备
    seq: 0, epoch: 'server', sessionId: sessionId ?? null, instanceId: viewingInstanceId, cwd: viewingCwd,
    ts: Date.now(), type: 'mirror_state', payload: { readonly, stale }
  });
}
...
async function catchUpTick() {
  ...
  mirrorRelease = rel.state;
  setMirror(rel.readonly, a.sessionId, false,                       // setMirror 仅在锁/stale 态变化时广播（解锁瞬间→前端恢复输入）
