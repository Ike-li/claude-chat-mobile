...
import { resolveFreshPrefs, defaultsFromEffectiveSettings } from './cli-settings-defaults.js';
...
import { buildWebStatusLine, buildCliStatusLine } from './statusline.js';
import { readCliObservedState } from './cli-mirror-state.js';
import { readCliStatusSnapshot, selectStatusOwner, selectStatusReplay, selectStatusSource } from './cli-statusline-bridge.js';
...
const {
  ...
  // 完全空闲（无在途轮/后台任务/挂起审批提问）的 web 实例回收阈值；默认 30min。0 = 禁用。
  // 与 IDLE_TIMEOUT_MS（在途轮静默挂死）正交：后者只在 pendingTurns>0 时生效。
  INSTANCE_IDLE_RECLAIM_MS = 1800000,
...
// 0 合法 = 禁用空闲回收；非法/缺省 → 30min
const instanceIdleReclaimMs = (() => {
  const n = Number(INSTANCE_IDLE_RECLAIM_MS);
  if (INSTANCE_IDLE_RECLAIM_MS === '' || INSTANCE_IDLE_RECLAIM_MS == null) return 1_800_000;
  if (!Number.isFinite(n) || n < 0) return 1_800_000;
  return n;
})();
