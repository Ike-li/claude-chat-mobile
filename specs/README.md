...
- Client-side attachment boundary checks for oversized files, total size, repeated same-file selection, and unknown file types; these do not upload to the real server or touch Claude. The visual mock server echoes only attachment metadata (`name`, `mimeType`, `size`, `thumb`) in `user_message`, never full base64 data.
