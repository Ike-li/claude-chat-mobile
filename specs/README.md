...
- Session navigation coverage verifies an unsent text draft is cleared on session switch so it cannot be sent to the wrong session.
...
- Attachment echo coverage verifies the visual mock server returns only attachment metadata (`name`, `mimeType`, `size`, `thumb`) in `user_message`, never full base64 data.
...
- `scripts/visual-mock-scenarios.js` contains the reusable visual mock scenario registry for incremental `test:*` command migration.
