...
- P0 count: 99 tests in 20 files
- Browser error capture: all 99 P0 tests call `expectNoBrowserErrors(page)`
...
1. P0-03d Markdown sanitization
   - Why: chat renders model output as Markdown/HTML-like content, so unsafe tags or event handlers must not become executable UI.
   - Test shape: add a mock stream containing script-like HTML, image `onerror`, and safe markdown; assert dangerous content is not executable/visible as active markup and the safe markdown still renders.
   - Mock impact: add a `test:unsafe-markdown` visual mock path.
   - Priority: high.

2. P0-06g permission sheet backdrop and draft guard
   - Why: AskUserQuestion has backdrop/draft guard coverage; permission approval should have the same must-answer behavior.
   - Test shape: open `test:permission`, fill a background draft, tap backdrop, assert permission sheet stays open, draft stays, no user message is sent, then allow/deny to finish.
   - Mock impact: none.
   - Priority: high.

3. P0-17g delayed terminal read-only lock after session switch
...
4. P0-18g attachment removal frees quota
...
5. P0-11r closing the last visible session falls back to empty start
...
6. P0-16e console modal clear after new logs arrive
...
7. P0-20e token retry does not leak rejected token into local UI state
...
8. P0-19d narrow viewport scroll reachability for settings and console sheets
...
1. Add P0-03d Markdown sanitization.
2. Add P0-06g permission sheet backdrop/draft guard.
3. Add P0-17g delayed mirror lock after session switch.
4. Refactor sidebar helpers before adding more P0-11 cases.
5. Add P0-18g attachment removal frees quota.
6. Add P0-11r close-last-session empty fallback.
7. Add P1 contract drift check.
8. Refactor visual mock scenario registry.
