...
// 这些是 docs/design.md 明列要做的。OQ-09 拒绝的是"授权闭环时延"这类**人机/价值指标**的埋点遥测，不在此列
// （见 docs/design.md 原文对二者的区分）；本模块不采集任何审批时延/闭环时长。

const counters = new Map(); // name → number（累计，只增）
const gauges = new Map();   // name → number（瞬时，覆盖式）

// 计数器递增（docs/design.md MetricsCollector.inc）。用于"发生过 N 次"类指标：事件数、限速锁定、推送成功/失败。
export function inc(name, delta = 1) {
  counters.set(name, (counters.get(name) || 0) + delta);
}

// 瞬时值设置（docs/design.md MetricsCollector.gauge）。用于需要"事件驱动更新、消费者随时读最新"的瞬时指标。
...
// 服务状态可见性（见 docs/design.md「用户可观察状态」）——刻意不复用 classifyState：
