...
// 列表真相源是扫盘（history.js#listSessionsPage），本模块只存体验增强元数据（标题缓存/指针）。
// 曾有的 hiddenSessionIds（L1 软隐藏）已移除——它制造 CLI/web 不等价且无反隐藏入口。
...
function load() {
  ...
  return {
    ...
    // 旧 sessions.json 若仍带 hiddenSessionIds，load 时直接丢弃；下次 save 不再写出。
  };
}
...
//
// 原 L1 hideSession/unhideSession/isHidden/getHiddenIds 已删（2026-08-26）：软隐藏制造终端不等价。
// 彻底删除走 session:deletePermanent；删文件窗口的临时列表排除用进程内 pendingDeleteIds（app.js）。
