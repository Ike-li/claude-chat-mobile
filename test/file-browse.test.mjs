// test/file-browse.test.mjs —— FileBrowseHandler 单测（LLD §3.4.2，承接 AD-12/FR-07"浏览项目文件"）
// listDir/readFile：请求-响应型只读文件浏览，弱网上限（分页/截断）、二进制检测、symlink 如实标注。
// 敏感文件（.env 等）不做内容过滤——LLD 显式抉择（机主即 root + 透明性，防线在范围门不在内容审查），
...
import { listDir, readFile, MAX_BROWSE_ENTRIES, MAX_BROWSE_BYTES } from '../file-browse.js';
