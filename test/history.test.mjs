...
import { getProjectDir, listSessions, listSessionsPage, sessionFileExists, getSessionHistory, HISTORY_MAX_MESSAGES, catchUpStep, sessionFileMtime } from '../history.js';
...
// ── sessionFileMtime：切入即判活的 mtime 探测 ────────────────────────────────

test('sessionFileMtime: 存在的会话 → 返回正 mtimeMs', async () => {
  const cwd = '/proj/mtime';
  writeJSONL(join(BASE, getProjectDir(cwd)), 'live', [{ type: 'user', message: { content: 'hi' } }]);
  const mt = await sessionFileMtime('live', cwd, { baseDir: BASE });
  assert.ok(mt > 0, 'mtimeMs 应为正');
});

test('sessionFileMtime: 不存在的会话 → 0', async () => {
  assert.equal(await sessionFileMtime('nope', '/proj/mtime', { baseDir: BASE }), 0);
});
