...
import { getProjectDir, listSessions, listSessionsPage, sessionFileExists, sessionFileSize, sessionFileMtime, getSessionHistory, HISTORY_MAX_MESSAGES, catchUpStep, rebaselineAbsorbedExternal, classifyTranscriptTail, lastPermissionMode, readLastPermissionMode, lastMessageActivityMs, __setSdkListSessionsForTest } from '../history.js';
...
test('SDK 快路径: 字段映射 id/title/lastUsedAt，dir 传原始 cwd，不返回 model/entrypoint', async () => {
  ...
  try {
    ...
    // ③ 字段映射（假 id 无真实 jsonl → lastUsedAt 回落 lastModified）
    assert.equal(sessions.length, 2);
