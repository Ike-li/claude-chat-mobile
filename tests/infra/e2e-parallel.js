...
//
// 【改成按时长 LPT 分配后】同一批 270 条用例实测（2026-09-07）：
//   4 分片：167s → **140.8s**，全绿。
//   8 分片：**88s**，但当时**会偶发假红**：3 轮里 87.8s 全绿 / 91.8s 红 1 条 / 124.7s 红 1 条，
//   红的是 task-progress 的 P0-17k、P0-17i——并发抬高后 CPU 争抢把它们的断言窗口挤没了。
//   **假红比慢更糟**，所以缺省一度停在 4。两条后来都修掉了：P0-17k 改用 -hold 变体（见该用例注释），
//   P0-17i 把 mock 的定时三态改成由用例显式推进（见 status.js 的 test:mirror），不再有窗口可赌。
//
// 【缺省 = min(8, 核数 - 2)】2026-09-23 实测（10 核 · 52 个 spec · 345 条 · 串行总和 851s）：
//   4 分片 206.6s；8 分片 115.8 / 104.5 / 105.9s，三轮 1035 条零失败零 flaky。
//   4 分片时 user+sys 434s / real 207s，只用了 2.1 个核——瓶颈是分片数，不是 CPU。
//   「核数 - 2」给小机器留余量：8 核 6 片、4 核 2 片（这两档没实测）。遇到疑似时序假红，
//   先用 CCM_E2E_SHARDS=4 重跑对照，再判断是不是真 bug。
//
// 【地板是最大那个 spec 文件自己的耗时】同一个 spec 文件不能跨分片，所以它是任何分片数都突破
// 不了的下限：2026-09-23 是 workspace-sessions-sidebar（46 条，4 分片全量里观测约 107s）。
// 8 分片的理想值 851/8 ≈ 106s 已经贴着它，再加片没有收益；再往下只能拆这个文件——那是
// 重组测试内容，不是调编排参数。
//
// ── CI 横向分片（2026-09-13）────────────────────────────────────────────
...
// 缺省 min(8, 核数 - 2)：8 以上被地板卡住没有收益，「- 2」给小机器留余量（数字与理由见文件头）。
