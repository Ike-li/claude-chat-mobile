...
//     已在 tests/v2/file-browse.test.mjs 与 workdir-scope-guard.test.mjs（S1）。
...
const TOKEN = 'v2-files-scope-token';

let dir, workdir, auditFile, server, sock;

test.before(async () => {
  dir = mkdtempSync(join(tmpdir(), 'ccm-v2-files-'));
  ...
  sock = ioClient(`http://127.0.0.1:${server.port}`, {
    auth: { token: TOKEN, deviceToken: 'v2-files-device' },
