...
// ---- turn-end 文件变更汇总（本轮 Edit/Write 聚合）----
test('isFileMutationTool: 写盘工具 true，Read 与无关工具 false', () => {
  assert.equal(isFileMutationTool({ name: 'Edit', changeKind: 'edit' }), true);
  assert.equal(isFileMutationTool({ name: 'Write', changeKind: 'write' }), true);
  assert.equal(isFileMutationTool({ name: 'MultiEdit', changeKind: 'multiedit' }), true);
  assert.equal(isFileMutationTool({ name: 'NotebookEdit', changeKind: 'notebook' }), true);
  assert.equal(isFileMutationTool({ name: 'Read', changeKind: 'read' }), false);
  assert.equal(isFileMutationTool({ name: 'Bash' }), false);
  assert.equal(isFileMutationTool({}), false);
});

test('countContentLines / estimateMutationLineStats: 行数估算', () => {
  assert.equal(countContentLines(''), 0);
  assert.equal(countContentLines(null), 0);
  assert.equal(countContentLines('a'), 1);
  assert.equal(countContentLines('a\nb\nc'), 3);
  assert.deepEqual(
    estimateMutationLineStats('Edit', { old_string: 'a\nb', new_string: 'a\nb\nc\nd' }),
    { added: 4, removed: 2 },
  );
  assert.deepEqual(
    estimateMutationLineStats('Write', { content: 'line1\nline2' }),
    { added: 2, removed: 0 },
  );
  assert.deepEqual(
    estimateMutationLineStats('MultiEdit', {
      edits: [
...
test('accumulateTurnFileChange + summarizeTurnFileChanges: 聚合同路径、忽略 Read', () => {
  const map = new Map();
  accumulateTurnFileChange(map, {
    path: '/repo/CLAUDE.md', name: 'Write', changeKind: 'write', toolUseId: 't1', added: 77, removed: 0,
  });
  accumulateTurnFileChange(map, {
    path: '/repo/README.md', name: 'Edit', changeKind: 'edit', toolUseId: 't2', added: 2, removed: 2,
  });
  // Read 不进账
  accumulateTurnFileChange(map, {
    path: '/repo/CLAUDE.md', name: 'Read', changeKind: 'read', toolUseId: 't3', added: 99, removed: 99,
  });
  // 同文件再 Edit：累加 +/-，toolUseId 更新为最新
  accumulateTurnFileChange(map, {
    path: '/repo/CLAUDE.md', name: 'Edit', changeKind: 'edit', toolUseId: 't4', added: 2, removed: 2,
  });
  const sum = summarizeTurnFileChanges(map);
  assert.equal(sum.fileCount, 2);
  assert.equal(sum.added, 81); // 77+2+2
  assert.equal(sum.removed, 4); // 0+2+2
  assert.equal(sum.title, '已编辑 2 个文件');
  assert.equal(sum.statsLabel, '+81 -4');
  const claude = sum.files.find(f => f.baseName === 'CLAUDE.md');
  assert.equal(claude.toolUseId, 't4');
  assert.equal(claude.added, 79);
