...
// 这份清单会随文档一起进分发 tarball，而分发包是仓库的裁剪子集（tests/ 与部分 scripts/
// 被 export-ignore 掉）。不说明的话，包内读者看到「Total classified files: 506」却只找得到
// 213 项，会以为自己拿到的包缺东西——文档在对它的读者说一件不为真的事。
test('生成的清单要声明自己描述的是完整仓库，不是分发包', () => {
  const rendered = renderRepositoryMap(['package-lock.json', 'app/server.js']);
  assert.match(rendered, /distribution|tarball|分发/i, '要点明分发包是它的子集');
});

test('repository map records entry path and generation source for every file', () => {
  const rendered = renderRepositoryMap(['package-lock.json', 'app/server.js', 'app/src/server/app.js']);
  ...
  assert.match(rendered, /`app\/server\.js`[^\n]+`node app\/server\.js`/);
  assert.match(rendered, /`app\/src\/server\/app\.js`[^\n]+Imported by the runtime/);
});
