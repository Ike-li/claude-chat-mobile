...
test.describe('isSupervised', () => {
  test('launchd 托管 → true（实测 XPC_SERVICE_NAME 确实被注入）', () => {
    assert.equal(isSupervised({ XPC_SERVICE_NAME: 'com.ccm.server' }), true);
  });

  test('systemd 托管 → true（INVOCATION_ID 或 JOURNAL_STREAM）', () => {
    assert.equal(isSupervised({ INVOCATION_ID: 'abc123' }), true);
    assert.equal(isSupervised({ JOURNAL_STREAM: '8:12345' }), true);
  });

  test('前台 npm start → false（停了就再也起不来，绝不能放行重启）', () => {
    assert.equal(isSupervised({}), false);
  });

  // ★ 实测抓到的陷阱：macOS 给**所有**普通进程也注入 XPC_SERVICE_NAME，值是字面量 "0"
  // （node -e 里实见）。只判非空的话，前台 npm start 会被认成受管 —— 正是要堵的那个洞。
  test('macOS 普通终端进程的 XPC_SERVICE_NAME=0 → false', () => {
    assert.equal(isSupervised({ XPC_SERVICE_NAME: '0' }), false);
    assert.equal(isSupervised({ XPC_SERVICE_NAME: ' 0 ' }), false);
  });

  test('空串按未设置处理（同 config.js 的 normalizeLoadedEnvironment）', () => {
    assert.equal(isSupervised({ XPC_SERVICE_NAME: '' }), false);
    assert.equal(isSupervised({ INVOCATION_ID: '   ' }), false);
  });
});
