...
const SOURCE_FILES = [...walk('app/src'), ...walk('scripts')];
...
test('BIND_MODE 的合法值在 bind-host 与 env-schema 之间不许漂移', async () => {
  const { BIND_MODES } = await import('../../app/src/shared/bind-host.js');
...
test('默认端口只在 env-schema.js 定义一次', () => {
  ...
  const re = /(PORT[^\n]*\b3000\b|\b3000\b[^\n]*PORT|\?\?\s*3000\b)/;
  const hits = findInSources(re, ['app/src/ops/env-schema.js']);
  assert.deepEqual(hits, [], '用 env-schema.js 的 DEFAULT_PORT，别再写字面量 3000');
});

// ── ③′ 源文件必须是 grep 看得见的纯文本 ──────────────────────────────────
// 这条是上面所有闸的**前提条件**。app/src/sessions/session-registry.js 里曾有一个裸 NUL 字节
...
test('源文件不含裸 NUL 字节（否则整个文件对 grep 与 git diff 都隐形）', () => {
  ...
  const files = SOURCE_FILES
    .concat(walk('app/public/js'), walk('desktop', ['.swift']), walk('tests', ['.mjs', '.ts', '.js']));
...
test('~/.claude 的路径拼装只在 app/src/shared/claude-home.js', () => {
  const hits = findInSources(/['"]\.claude['"]/, ['app/src/shared/claude-home.js']);
  assert.deepEqual(hits, [],
...
test('上传错误文案不硬编码 MB 数（由常量算）', () => {
  const hits = codeLines(read('app/src/files/uploads.js'))
    .filter(({ line }) => /上限\s*\d+\s*MB|limit\s+\d+\s*MB/i.test(line))
    .map(({ line, no }) => `app/src/files/uploads.js:${no}  ${line.trim()}`);
...
test('声称「与对方同语义」的实现都已进对照闸', () => {
  ...
  const claims = [];
  for (const file of SOURCE_FILES.concat(walk('app/public/js/logic'))) {
    for (const { line, no } of codeLines(read(file))) {
