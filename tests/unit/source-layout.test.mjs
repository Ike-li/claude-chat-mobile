...
const layout = {
  ...
  ops: [
    ...
    // config.js 曾在 app/src/server/ 下，但它是叶子（只 import ops/config-file.js、ops/env-schema.js、
    // shared/data-dir.js），放在 server/ 纯属历史，ops/ 才是叶子运维模块的归属地。
    // 附带效果：scripts/device.js 与 scripts/doctor.js 不再指向 app/src/server/。注意那条边**本来就是
...
test('backend domain modules live under src and not in the repository root', () => {
  for (const [domain, files] of Object.entries(layout)) {
    for (const file of files) {
      ...
      assert.equal(existsSync(`app/src/${domain}/${file}`), true, `${file} must live in app/src/${domain}`);
    }
  }
});
...
test('配置面板与安全体检都必须拿到 shell env 快照（少接一根线 = 静默假绿）', () => {
  const source = readFileSync('app/src/server/app.js', 'utf8');
