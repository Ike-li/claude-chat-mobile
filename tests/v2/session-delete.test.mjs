...
function harness(opts = {}) {
  ...
  return { ctl, emitted, bars, deleted, confirms, context };
}
...
test.describe('下发与回执', () => {
  ...
  // 失败路径的两条必须一起钉：只钉「提示了错误」的话，把 onDeleted 挪出 else 分支不会红
  // ——那正是「后端拒绝了，前端却把这一条从列表里抹掉」的形态。
  test('ok:false → 用 danger class 显示后端错因，且【不】调 onDeleted', async () => {
    const h = harness({ res: { ok: false, error: '会话正在被驱动，拒绝删除' } });
    await h.ctl.openDeleteSession('s-1', '/w/a', '重构分支');
    assert.equal(h.bars.length, 1);
    assert.equal(h.bars[0].text, '会话正在被驱动，拒绝删除', '后端错因必须原样透传，不得吞成通用文案');
    assert.equal(h.bars[0].cls, 'text-danger');
  ...
  test('回执为 undefined（连接断了/超时无响应）→ 走失败分支的兜底文案', async () => {
    ...
    assert.equal(h.bars.length, 1);
    assert.equal(h.bars[0].cls, 'text-danger');
    assert.ok(h.bars[0].text.length > 0, '没有 error 字段时也得给一句话，不能显示 undefined');
    assert.deepEqual(h.deleted, []);
  });
  ...
  test('ok:false 且无 error 字段 → 同样落兜底文案', async () => {
    ...
    assert.equal(h.bars.length, 1);
    assert.equal(h.bars[0].cls, 'text-danger');
    assert.notEqual(h.bars[0].text, 'undefined', '错误文案不得把 undefined 直接渲染出去');
    assert.deepEqual(h.deleted, []);
  });
});
